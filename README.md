...
Shapely (which requires GEOS), NumPy and Matplotlib. Run the file in a python terminal, with the input gcode file in a folder "gcode" in the same directory as the customsupportinjector script. The current version also only reads OrcaSlicer gcode files correctly. The script looks at certain types of signposting for the generation of the gcode, and the signposting is slightly different for different slicers (TYPE: Outer-wall or TYPE: WALL-OUTER)
